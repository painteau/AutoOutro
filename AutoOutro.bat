...
%mypath%\files\ffmpeg -i %mypath%\files\intermediaire.mp4 -i %mypath%\files\outro%sec%s.mp4 -filter_complex "[0:v] [0:a] [1:v] [1:a] concat=n=2:v=1:a=1 [v] [a]" -map "[v]" -map "[a]" %mypath%\FINAL-%madate%.mp4
